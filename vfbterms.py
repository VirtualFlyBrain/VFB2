    ...
    for k, v in zip(src.keys(), src.values()):
        if key == k:
            if not v in str(dest):
                dest.add('<img src="' + v + 'thumbnail.png" alt="{{< param linkTitle >}}" width="100"/>')
...
chdir(mypath + 'vfb/')
save_terms(vc.nc.commit_list(["MATCH (n:Individual) WHERE n.short_form starts with 'VFB_' AND NOT n:Deprecated WITH n.short_form as id ORDER BY id ASC RETURN collect(distinct id) as ids"])[0]['data'][0]['row'][0])
